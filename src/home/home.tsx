    ...
    onSnapshot(collection(firestore, "home"), (snapshot) => {
      setFeeds(
        snapshot.docs
          ...
          .sort((b, a) =>
            b.data().time.nanoseconds > a.data().time.nanoseconds
              ? -1
              : b.data().time.nanoseconds === a.data().time.nanoseconds
              ? 0
              : 1
          ...
          .map((feed) => {
            ...
            return {
              ...
              type: feed.data().type,
              time:
                dateTime.toLocaleTimeString() + " " + dateTime.toDateString(),
            };
          })
      );
  ...
  return (
    <div id="home-container">
      {selectedFeed === null ? (
              ...
              onClick={() =>
                setCategory(
                  category.includes("text")
                    ? category.filter((e) => e !== "text")
                    : category.concat("text")
                )
              }
              variant={category.includes("text") ? "filled" : "outlined"}
            />
              ...
              onClick={() =>
                setCategory(
                  category.includes("video")
                    ? category.filter((e) => e !== "video")
                    : category.concat("video")
                )
              }
              variant={category.includes("video") ? "filled" : "outlined"}
            />
          </div>
          {loading ? (
        ...
        <div id="home-selected-feed-outer">
          <div
            style={{
              display: "flex",
              position: "relative",
          ...
          >
            <h1 style={{ textAlign: "center", maxWidth: "75vw" }}>
              {selectedFeed.title}
            </h1>
            <img
